...
@export var col_hover : Color = Color.LIGHT_PINK
@export var col_unhover : Color = Color.WHITE
var par : Control
...
func _on_hover() -> void:
	...
	t.tween_property(par, "modulate", col_hover, duration)
	t.tween_property(par, "scale", Vector2.ONE * 1.1, duration)
func _on_unhover() -> void:
	var t : Tween = create_tween().set_ease(Tween.EASE_OUT)
	t.set_trans(Tween.TRANS_QUINT).set_parallel(true)
	t.tween_property(par, "modulate", col_unhover, duration)
